...
/*	if (effectiveSpecial.contains("creature"))
	...
	// we roll 0-2000 if that number is < quality * 100 i.e. VHQ 100 VLQ 700 if we get less than the odds we dont stick the special
...
void GeneticLabratory::setInitialCraftingValues(TangibleObject* prototype, ManufactureSchematic* manufactureSchematic, int assemblySuccess) {
	...
	if (phy == nullptr || pro == NULL || men == nullptr || psy == nullptr || agr == nullptr)
		return;
	...
	saberMax = Genetics::resistanceFormula(phy,pro,men,psy,agr,SharedWeaponObjectTemplate::LIGHTSABER,35.0f);
	...
	for (int i = 0; i < craftingValues->getExperimentalPropertySubtitleSize(); i++) {
		...
		if (title == "fortitude") {
			fortitudeBase = currentPercentage * maxValue;
		}
	}

	int armorRating = (fortitudeBase < 500) ? 0 : 1;
	effectiveness = (int)(((fortitudeBase - (armorRating * 500)) / 50.0f) * 11);
...
void GeneticLabratory::experimentRow(CraftingValues* craftingValues,int rowEffected, int pointsAttempted, float failure, int experimentationResult){
	...
	for (int i = 0; i < craftingValues->getExperimentalPropertySubtitleSize(); ++i) {
		...
		if (subtitlesTitle == title) {
			...
			if (experimentationResult == CraftingManager::CRITICALFAILURE) {
				...
				craftingValues->setCurrentPercentage(screwedTitle,current * (-0.05)); // reduce a random attribute by 5%
			}
	...
	int armorRating = (currentFort < 500) ? 0 : 1;
	float currentEffective = (int)(((currentFort - (armorRating * 500)) / 50.0f) * 11);
