...
public:
	...
	static float resistanceFormula(DnaComponent* a, DnaComponent* b, DnaComponent* c, DnaComponent* d, DnaComponent* e, int type, int max) {
		...
		switch(type) {
			case SharedWeaponObjectTemplate::ACID:
				any = hasVulnerability(a->getAcid(),b->getAcid(),c->getAcid(), d->getAcid(), e->getAcid());
			...
			case SharedWeaponObjectTemplate::BLAST:
				any = hasVulnerability(a->getBlast(),b->getBlast(),c->getBlast(), d->getBlast(), e->getBlast());
			...
			case SharedWeaponObjectTemplate::COLD:
				any = hasVulnerability(a->getCold(),b->getCold(),c->getCold(), d->getCold(), e->getCold());
			...
			case SharedWeaponObjectTemplate::ELECTRICITY:
				any = hasVulnerability(a->getElectric(),b->getElectric(),c->getElectric(), d->getElectric(), e->getElectric());
			...
			case SharedWeaponObjectTemplate::ENERGY:
				any = hasVulnerability(a->getEnergy(),b->getEnergy(),c->getEnergy(), d->getEnergy(), e->getEnergy());
			...
			case SharedWeaponObjectTemplate::HEAT:
				any = hasVulnerability(a->getHeat(),b->getHeat(),c->getHeat(), d->getHeat(), e->getHeat());
			...
			case SharedWeaponObjectTemplate::KINETIC:
				any = hasVulnerability(a->getKinetic(),b->getKinetic(),c->getKinetic(), d->getKinetic(), e->getKinetic());
			...
			case SharedWeaponObjectTemplate::LIGHTSABER:
				any = hasVulnerability(a->getSaber(),b->getSaber(),c->getSaber(), d->getSaber(), e->getSaber());
			...
			case SharedWeaponObjectTemplate::STUN:
				any = hasVulnerability(a->getStun(),b->getStun(),c->getStun(), d->getStun(), e->getStun());
	...
	static uint32 initialValue(float maxValue) {
		return round(maxValue * ((maxValue/(float)1000)+0.45));  // .15 to .45
	}
	...
	static int generateRegenLevel(int avgHam) {
		return (DnaManager::instance()->levelForScore(DnaManager::REG_LEVEL,avgHam/10) + 1)* 2;
	}
	static int generateArmorLevel(int armor, float effectResist) {
	...
	static int calculateAgentLevel(int health, float dps, float hit, int regen, int armor, float effective, float kin, float eng, float bla, float heat, float cold, float elec, float acid, float stun) {
		...
		int regenerationLevel =  generateRegenLevel(health);
		// Logger::console.error("Armor = " + String::valueOf(armor) + " effective = " + String::valueOf(effective));
		int armorLevel = generateArmorLevel(armor,effective);
		...
		/* Logger::console.error("Agent statLevel/damageLevel/hitLevel/regenerationLevel/armorLevel/armorBase/baseLevel/armorLevel2/defenseLevel = " + 
		  String::valueOf(statLevel) + "/" + String::valueOf(damageLevel) + "/" + String::valueOf(hitLevel) + "/" + String::valueOf(regenerationLevel) + "/" +
		  String::valueOf(armorLevel) + "/" + String::valueOf(armorBase) + "/" + String::valueOf(baseLevel) + "/" + String::valueOf(armorLevel2) + "/" + String::valueOf(defenseLevel));  */

		int level = Math::max((float)floor((((float)(statLevel + damageLevel/3.0f +  hitLevel + defenseLevel + armorLevel*2.0 + regenerationLevel))/18.1f)) -12, 1.0f);
		//Logger::console.error("Agent level = " + String::valueOf(level));
		return level;
	}
	...
	static int calculatePetLevel(GeneticComponent* pet) {
		...
		// Logger::console.error("pet->getArmor() * 500 = " + String::valueOf(pet->getArmor() * 500) + " pet->getEffectiveArmor() *10 = " + String::valueOf(( pet->getEffectiveArmor()) * 10.0));
		...
		/*Logger::console.error("Pet avgHam/statLevel/damageLevel/hitLevel/regenerationLevel/armorLevel/armorBase/baseLevel/armorLevel2/defenseLevel = " + 
		  String::valueOf(avgHam) + "/" + String::valueOf(statLevel) + "/" + String::valueOf(damageLevel) + "/" + String::valueOf(hitLevel) + "/" + String::valueOf(regenerationLevel) + "/" +
		  String::valueOf(armorLevel) + "/" + String::valueOf(armorBase) + "/" + String::valueOf(baseLevel) + "/" + String::valueOf(armorLevel2) + "/" + String::valueOf(defenseLevel)); */

		int level = Math::max((float)floor((((float)(statLevel + damageLevel/3.0f + hitLevel + defenseLevel + armorLevel*2.0 + regenerationLevel))/18.1f)) - 12, 1.0f);
		//("Pet level = " + String::valueOf(level));
		return level;
	}
