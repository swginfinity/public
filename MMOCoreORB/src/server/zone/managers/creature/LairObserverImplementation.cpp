...
bool LairObserverImplementation::checkForNewSpawns(TangibleObject* lair, TangibleObject* attacker, bool forceSpawn) {
	...
	//error("Spawn number = " + String::valueOf(spawnNumber));
	if (spawnNumber == 4) {
		if (System::random(100) > 40)  {//40% chance of boss mobile on wild lairs
			//error("No boss mobile this lair.");
			return false;
		}
		//error("Spawning boss mobile...");
