...
public:
	...
	void run() {
		...
		if (lootAll) {
			...
			if (group->getLootRule() == GroupManager::MASTERLOOTER && lootContainer->getContainerObjectsSize() <= 0) {
				...
				group->sendSystemMessage(msg, false, 2);   //messageType 1 = Harvest, 2 = Loot
			}
		} else {
			gclocker.release();
	...
	void splitCredits() {
		...
		group->sendSystemMessage(lootMember, player, 2);  //messageType 1 = Harvest, 2 = Loot
		...
		for (int i = 0; i < payees.size(); ++i) {
			...
			String typeSelected = "";
			ManagedReference<PlayerObject*> payeeGhost = payee->getPlayerObject();
			if (payeeGhost != nullptr)	{
				typeSelected = payeeGhost->getScreenPlayData("group_message_selection", "typeSelected");
			}
			//Send credit split system message.

			//Logger::console.error("Payee = " + payee->getFirstName() + ", typeSelect = " + typeSelected);

			//if (typeSelected == "All" || typeSelected == "" || typeSelected == "Loot Only") {

				if (payee == player) {
					if (payees.size()  > 1) {   // Only send message if there is a split
				...
				} else {   //Split with in-range group members
