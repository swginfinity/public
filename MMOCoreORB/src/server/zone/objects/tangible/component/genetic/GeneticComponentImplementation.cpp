...
void GeneticComponentImplementation::updateCraftingValues(CraftingValues* values, bool firstUpdate) {
	...
	health = ((hardiness * 15)    + (dexterity * 3)) * 2.0;
	action = ((dexterity * 15)    + (intelligence * 3)) * 2.0;
	mind   = ((intelligence * 15) + (hardiness * 3)) * 2.0;
	stamina = ((dexterity*15)     + (endurance * 3)) * 2.0;
	willPower = ((intelligence * 15) + (cleverness * 3)) * 2.0;
	constitution = ((hardiness * 15)    + (fortitude * 3)) * 2.0;
	focus = ((intelligence * 15) + (dependency * 3)) * 2.0;
	strength = ((hardiness * 15)    + (dependency * 3)) * 2.0;
	quickness = ((dexterity * 15)    + (dependency * 3)) * 2.0;
	hit = 0.50 + (1.25 * ((float)cleverness/1000.0));
	// dps of pet use to determien min and max value.
	int dps = ceil(15.0 + (735.0 * (float)power /1000.0));
	//int dps = ceil((ceil(15.0 + (775.0 * ( ((float)power)/1000.0))))/3.5) * 2.0;  //Double DPS
	speed = Math::max(2.0, (2.4 - round((float)courage/1000.0 * 10) / 10));
	maxDam = round((float)dps * speed);
