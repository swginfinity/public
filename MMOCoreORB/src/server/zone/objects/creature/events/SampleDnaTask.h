...
public:
	...
	void run() {
		...
		int maxSamples = ceil((float)skillMod / 15.0f) - 1;
		...
		case END:
			...
			if (rollMod < 0) // Less penalty on sampling higher level creatures
				rollMod /= 10.0;
			
			//Logger::console.error("sampleRoll = " + String::valueOf(sampleRoll) + " rollMod = " + String::valueOf(rollMod));
			// We have the players roll. NOW to determine if success of failure;
			if (sampleRoll >= 50) { // adjust great success to 50% and above;
				...
				//Logger::console.error("DNA Sample Count = " + String::valueOf(creature->getDnaSampleCount()) + ", maxSamples = " + String::valueOf(maxSamples));
				if (creature->getDnaSampleCount() >= maxSamples ){
					creature->setDnaState(CreatureManager::DNASAMPLED);
					// We took the max samples the shock it too much and kils the creature.
					result = 4;
			...
			//Logger::console.error("Result = " + String::valueOf(result));
	...
	void award(int cl, float rollMod, int skillMod) {
		...
		int qualityRoll = luckRoll + rollMod/2;
