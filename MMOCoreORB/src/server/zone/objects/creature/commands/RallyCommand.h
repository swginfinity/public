...
//		leader->updateCooldownTimer("rally", (duration + 30) * 1000);
	...
	void sendRallyCombatSpam(CreatureObject* leader, GroupObject* group, bool success) const {
		...
		for (int i = 0; i < closeObjects.size(); ++i) {
			...
			if (object == nullptr || !object->isPlayerCreature() || group->hasMember(object->getObjectID()))
				continue;

			if(!object->isInRange(leader, 70.0)) // Skip any non group players beyond 70 meters (shouldn't happen)
				continue;
