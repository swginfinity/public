...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (container == "equipment") {
			...
			int frsXP = targetGhost->getExperience("force_rank_xp");

			String councilString;
			
			if (playerCouncil == 0)
				councilString = " is a Non-FRS Jedi of rank ";
			else if (playerCouncil == 1)
				councilString = " is a Light Jedi of rank ";
			else
				councilString = " is a Dark Jedi of rank ";
		
			creature->sendSystemMessage(targetObj->getCustomObjectName().toString() + councilString + String::valueOf(playerRank) + " with " + String::valueOf(frsXP) + " FRS experience.");

		} else if (container == "skills") {
			return sendSkillsInfo(creature, targetObj, args.hasMoreTokens());

		} else if (container == "experience" || container == "xp") {
			return sendExperienceInfo(creature, targetObj, args.hasMoreTokens());
	...
	int sendSkillsInfo(CreatureObject* creature, CreatureObject* target, bool eMail = false) const {
		...
		Locker locker(target,creature);

		SkillManager* skillManager = SkillManager::instance();
		SkillList* skillList = target->getSkillList();
        StringBuffer body;
		String targetName = target->getCustomObjectName().toString();
	...
	int sendExperienceInfo(CreatureObject* creature, CreatureObject* target, bool eMail = false) const {
		...
		for (int i = 0; i < xpCapList->size(); ++i) {
			...
			if (xpValue != 0)
				body << xpType <<  ": " << String::valueOf(xpValue) << "\n";
		}
